...
export function Checkout() {
    ...
    const handleConfirmOrder = () => {
        if (cartItems.length === 0) {
            setErrorMessage("Your cart is empry. Please add items before confirming order.")
        } else {
            clearCart();
    ...
    return (
        <div className={"my-10 font-title-bold"}>
            ...
            <div className={"bg-white flex flex-col items-center w-fit mx-auto p-5 font-light rounded min-w-[322px] md:min-w-[455px]"}>
                {cartItems.length > 0 ? (
                    <>
                                ...
                                <li key={item.id} className={"flex flex-col"}>
                                    <div className={"flex gap-4"}>
                                        ...
                                        <div className={"text-lg mt-2 flex flex-col gap-4"}>
                                            ...
                                            <div className={"underline underline-offset-4 cursor-pointer text-custom-black"}
                ...
                {errorMessage && <p className={"text-red-500 text-center mt-4 max-w-fit"}>{errorMessage}</p>}
            </div>
        </div>
    )
}